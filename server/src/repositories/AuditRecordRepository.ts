...
export class AuditRecordRepository {
  ...
  static async create(data: {
    ...
    const [result] = await pool.execute<ResultSetHeader>(
      `INSERT INTO audit_records
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [data.user_id, data.enterprise_id || null, data.task_id || null,
  ...
  // 根据ID查找
  static async findById(id: number): Promise<AuditRecord | null> {
    const [rows] = await pool.execute<RowDataPacket[]>(
      'SELECT * FROM audit_records WHERE id = ?', [id],
    );
    return rows[0] as AuditRecord || null;
  }
  ...
  static async findWithFilters(filters: {
    ...
    if (filters.user_id) { conditions.push('ar.user_id = ?'); params.push(filters.user_id); }
    if (filters.enterprise_id) { conditions.push('ar.enterprise_id = ?'); params.push(filters.enterprise_id); }
    if (filters.status) { conditions.push('ar.overall_status = ?'); params.push(filters.status); }
    if (filters.keyword) {
      conditions.push('(ar.file_name LIKE ?)');
      params.push(`%${filters.keyword}%`);
    }
    if (filters.start_date) { conditions.push('ar.created_at >= ?'); params.push(filters.start_date); }
    if (filters.end_date) { conditions.push('ar.created_at <= ?'); params.push(filters.end_date + ' 23:59:59'); }

    const where = conditions.length > 0 ? 'WHERE ' + conditions.join(' AND ') : '';

    const [[{ total }]] = await pool.execute<RowDataPacket[]>(`SELECT COUNT(*) as total FROM audit_records ar ${where}`, params);
    ...
    return { list: rows as AuditRecord[], total };
  }

  // 获取用户今日审核数量（用于统计）
  static async countTodayByUser(userId: number): Promise<number> {
    const [rows] = await pool.execute<RowDataPacket[]>(
      "SELECT COUNT(*) as count FROM audit_records WHERE user_id = ? AND DATE(created_at) = CURDATE()",
      [userId],
    );
