...
export class SubscriptionService {
  ...
  /** 获取所有可购买的套餐列表 */
  static async getAvailableTiers(): Promise<SubscriptionTier[]> {
    return await SubscriptionRepository.getActiveTiers();
  }

  /** 获取企业当前套餐状态 */
  static async getEnterpriseSubscriptionStatus(enterpriseId: number): Promise<{
    enterprise: Enterprise;
    tier: SubscriptionTier | null;
    isActive: boolean;
    isExpiring: boolean;      // 7天内到期
    daysRemaining: number;     // 剩余天数
    canIssueTasks: boolean;    // 今日是否还能下发任务
    tasksRemainingToday: number; // 今日剩余额度
    orderStats: { totalOrders: number; paidOrders: number; totalSpent: number };
  }> {
    const rows = await db.query(
      `SELECT e.*, t.name as tier_name, t.code as tier_code, t.price_yearly,
    ...
    if (!rows.length) throw new AppError('企业不存在', 404);
    ...
    const orderStats = await SubscriptionRepository.getOrderStats(enterpriseId);
  ...
  /** 获取企业订单列表 */
  static async getEnterpriseOrders(enterpriseId: number, page: number = 1, pageSize: number = 10) {
    return await SubscriptionRepository.getOrdersByEnterprise(enterpriseId, page, pageSize);
  }

  /** 获取用户订单列表 */
  static async getUserOrders(userId: number, page: number = 1, pageSize: number = 10) {
    return await SubscriptionRepository.getOrdersByUser(userId, page, pageSize);
  }
  ...
  static async createOrder(
    ...
    // 1. 校验目标套餐是否存在且上架
    const targetTier = await SubscriptionRepository.getTierById(tierId);
    if (!targetTier || !targetTier.is_active) {
      throw new AppError('目标套餐不存在或已下架', 400);
    }

    // 2. 查询当前企业状态
    const [ents] = await db.query(
      'SELECT * FROM enterprises WHERE id = ?', [enterpriseId]
    ) as Enterprise[];
    if (!ents) throw new AppError('企业不存在', 404);
    ...
    if (!currentTierId || !currentExpiresAt || currentExpiresAt < now) {
      ...
      // 查询当前套餐价格
      const currentTier = await SubscriptionRepository.getTierById(currentTierId!);
      ...
      if (discountAmount > 0) {
        throw new AppError('暂不支持降级操作，如需更换请先到期后重新购买', 400);
      }
    ...
    // 如果使用了积分，预检查余额
    if (pointsUsed > 0) {
      const [users] = await db.query('SELECT points FROM users WHERE id = ?', [userId]) as any[];
      if (!users || users.points < pointsUsed) {
        throw new AppError(`积分不足，需要 ${pointsUsed} 积分，当前余额 ${users?.points || 0}`, 400);
      }
    }

    // 5. 生成订单号并创建
    const orderNo = `SUB${Date.now()}${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
    const orderId = await SubscriptionRepository.createOrder({
      order_no: orderNo,
    ...
    return await SubscriptionRepository.getOrderById(orderId)!;
  }
   ...
   */
  static async handlePaymentSuccess(orderNo: string): Promise<SubscriptionOrder> {
    const order = await SubscriptionRepository.getOrderByNo(orderNo);
    if (!order) throw new AppError('订单不存在', 404);
    if (order.status === 'paid') throw new AppError('订单已支付', 400);

    // 扣减积分（如果使用了积分）
    if (order.points_used > 0) {
      await UserRepository.addPoints(order.user_id, -order.points_used);
      // 记录积分变动
    ...
    // 更新订单状态为已支付
    await SubscriptionRepository.updateOrderStatus(order.id, 'paid', {
      paid_at: new Date().toISOString().slice(0, 19).replace('T', ' '),
      ...(order.payment_method === 'mock' ? { trade_no: `MOCK_${Date.now()}` } : {}),
    });

    // 更新企业套餐状态
    await SubscriptionRepository.updateEnterpriseSubscription(order.enterprise_id, {
      tier_id: order.tier_id,
      subscription_started_at: order.period_start!,
      subscription_expires_at: order.period_end!,
      daily_task_limit: (order as any).daily_task_limit, // 来自JOIN的tier字段
      status: 'active',
    });

    // 如果之前是 frozen 状态，自动激活

    return await SubscriptionRepository.getOrderById(order.id)!;
  }

  /**
   * 模拟支付（开发/测试用）— 直接标记订单为已支付
   */
  static async mockPay(orderNo: string): Promise<SubscriptionOrder> {
   ...
   */
  static async checkTaskQuota(enterpriseId: number): Promise<{
    canIssue: boolean;
    remaining: number;   // -1 表示无限制
    limit: number;       // 0 表示无限制
    reason?: string;
  }> {
    const [ents] = await db.query(
      'SELECT tier_id, subscription_expires_at, daily_task_limit, tasks_issued_today, task_limit_reset_date, status FROM enterprises WHERE id = ?',
      [enterpriseId]
    ) as any[];

    if (!ents) return { canIssue: false, remaining: 0, limit: 0, reason: '企业不存在' };
    ...
    if (ents.task_limit_reset_date !== today) {
      issuedToday = 0;
      await SubscriptionRepository.updateEnterpriseSubscription(enterpriseId, {
  ...
  static async incrementTaskCount(enterpriseId: number): Promise<void> {
    const today = new Date().toISOString().slice(0, 10);
    await db.query(
      `UPDATE enterprises SET tasks_issued_today = tasks_issued_today + 1, task_limit_reset_date = ?
       WHERE id = ? AND task_limit_reset_date = ?`,
      [today, enterpriseId, today]
    );
  }
   ...
   */
  static async dailyReset(): Promise<{ enterprisesReset: number; frozenCount: number }> {
    const resetCount = await SubscriptionRepository.resetDailyTaskCounts();
    const frozenCount = await SubscriptionRepository.freezeExpiredEnterprises();
   ...
   * 获取即将到期的企业（用于续费提醒通知）
   */
  static async getExpiringEnterprises(days: number = 7) {
    return await SubscriptionRepository.getExpiringEnterprises(days);
  }
}
