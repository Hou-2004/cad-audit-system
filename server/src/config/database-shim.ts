...
const DB_FILE = path.resolve(process.cwd(), 'data', 'memory-db.json');
...
function parseValues(sql: string): unknown[] {
  // 提取 VALUES (?, ?, ?) 中的 ? 数量
...
// 简易 WHERE 解析
function buildWhere(conditions: string, params: unknown[]): (row: TableRow) => boolean {
  // 只支持简单的 AND 条件：col = ? 或 col IS NULL
  const parts = conditions.split(/\s+AND\s+/i);
  const resultFns: Array<(row: TableRow) => boolean> = [];
...
// 简易 SELECT 解析
function execSelect(sql: string, params: unknown[]): { rows: TableRow[]; fields: { name: string }[] } {
  const table = getTable(sql);
  if (!table || !tables[table]) return { rows: [], fields: [] };
  ...
  if (whereIdx !== -1) {
    ...
    const end = Math.min(
      limitIdx === -1 ? Infinity : limitIdx,
      orderIdx === -1 ? Infinity : orderIdx,
    );
...
// 简易 INSERT
function execInsert(sql: string, params: unknown[]): { insertId: number } {
  const table = getTable(sql);
  ...
  // 自动分配 ID
  const maxId = tables[table].reduce((max, r) => Math.max(max, (r.id as number) || 0), 0);
  row.id = maxId + 1;
...
// 简易 UPDATE
function execUpdate(sql: string, params: unknown[]): { affectedRows: number } {
  const table = getTable(sql);
  if (!table || !tables[table]) return { affectedRows: 0 };
...
class PreparedStatement {
  ...
  async execute(params: unknown[]): Promise<[ResultSetHeader | RowDataPacket[], unknown[]]> {
    return execute(this.sql, params);
  }
}

async function execute(sql: string, params: unknown[] = []): Promise<[ResultSetHeader | RowDataPacket[], unknown[]]> {
  const upper = sql.trim().toUpperCase();

  if (upper.startsWith('SELECT') || upper.startsWith('SHOW')) {
    const { rows, fields } = execSelect(sql, params);
    return [rows as RowDataPacket[], fields as unknown[]];
  }
