...
const AuditRecordRepository_js_1 = require("../repositories/AuditRecordRepository.js");
...
const AuditRuleRepository_js_1 = require("../repositories/AuditRuleRepository.js");
class AuditService {
    /**
     * 执行审核（工作者自行上传）
     */
    static async auditFileForWorker(userId, filePath, originalName, specRequirements, ruleIds) {
        const user = await UserRepository_js_1.UserRepository.findById(userId);
        if (!user)
            throw new Error('用户不存在');
        // 检查是否是企业直属员工（企业员工走不同流程）
        const employeeRelation = await UserRepository_js_1.UserRepository.findEmployeeRelation(userId);
        if (employeeRelation) {
        ...
        let pointsCost = 0;
        try {
            await UserRepository_js_1.UserRepository.useFreeAuditCount(userId);
            pointsCost = 0; // 免费
        }
        catch {
            // 免费用完，扣积分
            ...
            await UserRepository_js_1.UserRepository.deductPoints(userId, 1);
            pointsCost = 1;
        }
        // 执行审核
        const result = await this.performAudit(filePath, specRequirements, ruleIds);
        // 保存记录
        const recordId = await AuditRecordRepository_js_1.AuditRecordRepository.create({
            user_id: userId,
            file_name: originalName,
            file_path: filePath,
            file_size: fs_1.default.statSync(filePath).size,
     ...
     */
    static async auditFileForEmployee(userId, enterpriseId, filePath, originalName, specRequirements, taskId, ruleIds) {
        // 执行审核
        const result = await this.performAudit(filePath, specRequirements, ruleIds);
        // 更新任务分配状态（如果有任务）
        if (taskId) {
            await TaskRepository_js_1.TaskRepository.updateAssignmentStatus(taskId, userId, 'completed');
        }
        // 保存记录
        const recordId = await AuditRecordRepository_js_1.AuditRecordRepository.create({
            user_id: userId,
     ...
     * 核心审核逻辑 - 解析CAD文件并匹配规则
     */
    static async performAudit(filePath, specRequirements, ruleIds) {
        try {
            // 1. 解析 CAD 文件
            ...
            if (ruleIds && ruleIds.length > 0) {
                rulesToApply = await AuditRuleRepository_js_1.AuditRuleRepository.findByIds(ruleIds);
            }
            else {
                // 使用预设的全部规则
                rulesToApply = await AuditRuleRepository_js_1.AuditRuleRepository.findPreset();
            }
            // 合并用户自定义的规范要求
     ...
     * 解析 CAD 文件
     */
    static async parseCadFile(filePath) {
     ...
     */
    static async parseDxfFile(filePath, format, fileSize) {
                ...
                if (line === 'LAYER') {
                    ...
                    while (i + 1 < lines.length) {
                        ...
                        if (code === '2')
                            ...
                            layerData.linetype = val;
                        else if (code === '62')
                            layerData.color = parseInt(val) || 7;
                        else if (code === '70')
                            layerData.frozen = (parseInt(val) & 1) === 1;
                        else if (val === 'ENDTAB')
                            break;
                    }
                    if (layerData.name) {
     ...
     */
    static async parseDwgFile(filePath, format, fileSize) {
     ...
     * 从 DXF 内容中提取图框/标题栏信息
     */
    static extractTitleBlock(content) {
        // 查找常见的标题栏属性模式
        const patterns = [
            [/图纸[名称称][:：]\s*(\S+)/, 'title'],
            [/设\s*计[:：]\s*(\S+)/, 'designer'],
    ...
    // 规则检查引擎
    // ============================================================
    static async runChecks(cadInfo, rules, customSpec) {
        const items = [];
     ...
     * 根据单条规则执行检查
     */
    static checkAgainstRule(cadInfo, category, ruleConfig, _customSpec) {
        const items = [];
        switch (category) {
            case 'layer':
    ...
    // --- 各类检查实现 ---
    static checkLayers(cadInfo, config) {
        const issues = [];
        const warnings = [];
        // 检查必需图层是否存在
    ...
    static checkLinetypes(cadInfo, config) {
        const issues = [];
        if (config.allowedLinetypes) {
            for (const lt of cadInfo.linetypes) {
                const normalizedLt = lt.toUpperCase().replace(/[-_\s]/g, '');
                const allowedNormalized = config.allowedLinetypes.map((a) => a.toUpperCase().replace(/[-_\s]/g, ''));
                if (!allowedNormalized.some((a) => normalizedLt.includes(a) || a.includes(normalizedLt))) {
    ...
    static checkColors(cadInfo, config) {
        const issues = [];
        const warnings = [];
    ...
    static checkDimensions(cadInfo, config) {
        const warnings = [];
    ...
    static checkTextStyles(cadInfo, config) {
        const issues = [];
        const warnings = [];
        for (const ts of cadInfo.textStyles) {
            ...
            if (config.allowedFonts && ts.fontName) {
                ...
                const allowed = config.allowedFonts.some((a) => a.toLowerCase().replace(/[-_.]/g, '') === normalizedFont);
                if (!allowed) {
    ...
    static checkTitleBlock(cadInfo, config) {
    ...
    static async getAuditHistory(filters) {
        return await AuditRecordRepository_js_1.AuditRecordRepository.findWithFilters(filters);
    }
    /**
     * 获取审核详情
     */
    static async getAuditDetail(recordId) {
        return await AuditRecordRepository_js_1.AuditRecordRepository.findById(recordId);
    }
}
