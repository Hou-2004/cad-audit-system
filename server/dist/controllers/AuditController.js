...
class AuditController {
    // 上传并审核 CAD 文件（工作者）
    static async uploadAndAudit(req, res) {
        ...
        try {
            ...
            const ruleIds = req.body.rule_ids
                ? JSON.parse(req.body.rule_ids).map((id) => parseInt(id))
                : undefined;
            const result = await AuditService_js_1.AuditService.auditFileForWorker(userId, file.path, file.originalname, specRequirements, ruleIds);
            (0, response_js_1.success)(res, { recordId: result.recordId, auditResult: result.result }, '审核完成');
        }
        catch (err) {
    ...
    // 员工审核任务关联的CAD文件
    static async auditTaskFile(req, res) {
        const userId = req.user?.userId;
        ...
        try {
            // 获取任务的规范要求
            const { TaskRepository } = await Promise.resolve().then(() => __importStar(require('../repositories/TaskRepository.js')));
            ...
            const result = await AuditService_js_1.AuditService.auditFileForEmployee(userId, empRel?.enterprise_id || 0, file.path, file.originalname, specRequirements, taskId, ruleIds);
            (0, response_js_1.success)(res, { recordId: result.recordId, auditResult: result.result }, '审核完成');
        }
        catch (err) {
            res.status(400).json({ code: 400, message: err.message, data: null });
    ...
    static async getDetail(req, res) {
        ...
        if (userRole === 'super_admin') {
            record = await AuditService_js_1.AuditService.getAuditDetail(recordId);
        }
        else {
            record = await AuditService_js_1.AuditService.getAuditDetail(recordId);
            if (record && record.user_id !== userId) {
                ...
                if (!emp || emp.enterprise_id !== record.enterprise_id) {
                    res.status(403).json({ code: 403, message: '无权查看此记录', data: null });
                    return;
                }
            }
        }
        ...
        (0, response_js_1.success)(res, record);
    }
    // 审核历史列表
    static async getHistory(req, res) {
        ...
        const result = await AuditService_js_1.AuditService.getAuditHistory(filters);
        // 解析审核结果
        result.list.forEach((r) => {
            if (typeof r.audit_result === 'string' && r.audit_result)
                r.audit_result = JSON.parse(r.audit_result);
        });
